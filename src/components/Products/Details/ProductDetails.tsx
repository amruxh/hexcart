...
const ProductDetails = ({ productDetails }: { productDetails: Product | null }) => {
  ...
  const discountPercentage = Math.round(productDetails?.discountPercentage ?? 0);
  return (
      ...
      <div className="flex flex-col md:flex-row w-full">
        ...
        <div className="flex-1 mt-2 flex flex-col">
          ...
          <div className="rating flex order-3 gap-3 my-3">
            <span className="flex bg-green-600 text-[var(--foreground)] pl-1 py-0.5 rounded-xs text-sm items-center gap-1 w-[50px]">
          ...
          <div className="mt-6 order-6">
            <span>Status:{' '}
              <span className={`${productDetails?.availabilityStatus?.includes('Low') ? 'bg-red-800' : 'bg-green-700'} rounded-sm p-1 text-[var(--foreground)] box-border border border-[var(--border)]`}>{productDetails?.availabilityStatus}</span>
            </span>
          </div>
          {/* Buttons */}
